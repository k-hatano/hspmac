...
NSString* disasmString(unsigned short type,unsigned long code,unsigned char* data){
    switch(type){
        case TYPE_VAR:
        ...
        case TYPE_XCMD:
            ...
            break;
        case TYPE_PRGCMD:
            return [NSString stringWithFormat:@"%@",progcmds[code]];
            break;
        case TYPE_MARK:
            if(code==0x3f) return [NSString stringWithFormat:@"dum"];
            else if(code=='(') return [NSString stringWithFormat:@"("];
            else if(code==')') return [NSString stringWithFormat:@")"];
            else return [NSString stringWithFormat:@"%@",marks[code]];
            break;
        case TYPE_CMPCMD:
...
NSString* disasm(HSPCODE current,unsigned char* data){
    ...
    [res appendString:disasmString(type, current.code, data)];
...
+ (NSString*)disasmStringWithType:(int)type code:(int)code data:(unsigned char*)data{
    return disasmString(type,code,data);
}
